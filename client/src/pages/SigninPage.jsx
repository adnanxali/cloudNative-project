...
const SigninForm = () => {
  ...
  useEffect(() => {
    // Check if the token is valid when the app loads
    axios.get('http://localhost:3000/api/v1/employee/auth/check')
        .then(response => {
  ...
  const handleSignin = async () => {
    try {
      const response = await axios.post("http://localhost:3000/api/v1/employee/login", {
        email,
        password:pass,
      },
